...
function ITEM:Equip(client, bNoSelect, bNoSound)
	...
	if (IsValid(weapon)) then
		...
		print(Format("[Helix] Нельзя экипировать это оружие: %s. Оно не существует, как класс!", self.class))
	end
end
...
function ITEM:Unequip(client, bPlaySound, bRemoveItem)
	...
	if (IsValid(weapon)) then
		...
		print(Format("[Helix] Нельзя снять это оружие: %s. Оно не существует, как класс!", self.class))
	end
...
function ITEM:OnLoadout()
	if (self:GetData("equip")) then
		...
		if (IsValid(weapon)) then
			...
			print(Format("[Helix] Нельзя выдать это оружие: %s. Оно не существует, как класс!", self.class))
		end
	end
end
...
function ITEM:PopulateTooltip(tooltip)
    if (!self.entity) then
        ix.util.PropertyDesc(tooltip, "Оружие", Color(64, 224, 208))
        if self.weaponCategory == "secondary" then
        	ix.util.PropertyDesc2(tooltip, "Вешается на пояс", Color(64, 224, 208), Material("vgui/ui/stalker/weaponupgrades/handling.png"))
        end
        if self.weaponCategory == "primary" then
        	ix.util.PropertyDesc2(tooltip, "Вешается за спину", Color(64, 224, 208), Material("vgui/ui/stalker/weaponupgrades/handling.png"))
        end
    end

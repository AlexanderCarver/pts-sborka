...
if (CLIENT) then
	...
	function PLUGIN:HUDPaint()
		...
		if lclient:GetNWBool("ConstructablePropPlacing", false) then
			...
			if lclient:GetActiveWeapon():GetClass() == "ix_hands" then
				draw.SimpleText("ЛКМ: По горизонтали | ПКМ: По вертикали | " .. string.upper(useBind) .. ": Разместить | " .. string.upper(reloadBind) .. ": Выйти", "ConstructionPropFont", w/2, h/1.155, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			end
		end
	end
end

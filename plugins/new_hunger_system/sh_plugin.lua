...
--[[ Hunger ]]
ix.config.Add("hungrySeconds", 1100, "Голод, чем выше значение, тем медленее тратится голод", nil, {
...
--[[ Thirst ]]
ix.config.Add("thirstySeconds", 3000, "Жажда, чем выше значение, тем медленее тратится жажда", nil, {
	data = {min = 1, max = 99999},
	category = PLUGIN.name
})
...
--[[ Drunk ]]
ix.config.Add("drunkySeconds", 200, "Опьянение, чем выше значение, тем медленее проходит опьянение", nil, {
	data = {min = 1, max = 99999},
	category = PLUGIN.name
})

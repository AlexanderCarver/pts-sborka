ITEM.name = "Мятные леденцы"
ITEM.description = "Коробка, внутри которой лежат сладости.."
ITEM.longdesc = "Сладкое лакомство с освежающим мятным вкусом. Они содержат ментол, который обладает свойством охлаждать и освежать дыхание. Леденцы давно стали популярным средством для борьбы с неприятным запахом изо рта на Территории, да и иногда полезно закидывать в рот что-нибудь сладенькое. В общем, они достаточно популярны."
...
ITEM.functions.use = {
    ...
    OnRun = function(item)
        ...
        ix.util.PlayerPerformBlackScreenAction(item.player, "Употребление...", 4, function(player) 
			ix.chat.Send(player, "me", "достаёт коробку с леденцами и закидывает несколько штук себе в рот.")
		end)

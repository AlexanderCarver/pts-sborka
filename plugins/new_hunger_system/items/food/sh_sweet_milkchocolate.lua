ITEM.name = "Молочный шоколад"
ITEM.description = "Шоколадная плитка в упаковке."
ITEM.longdesc = "Плитка изготовлена из молочного шоколада, который обладает насыщенным молочным вкусом и нежной текстурой. Содержит большое количество какао-масла, которое придает шоколаду гладкость и приятный аромат. Идеальный выбор для любителей молочного шоколада и тех, кто хочет насладиться сладким удовольствием в любое время дня. Редкий экземпляр."
...
ITEM.functions.use = {
    ...
    OnRun = function(item)
        ...
        ix.util.PlayerPerformBlackScreenAction(item.player, "Употребление...", 4, function(player) 
			ix.chat.Send(player, "me", "достаёт молочный шоколад и закидывает несколько кусоков себе в рот.")
		end)

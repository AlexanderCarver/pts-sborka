...
Drunk.Say = { -- ic
	"Фу-ух!",
	"Хо-хорошо жмыхнуло...",
	"Тест системы опьянения"
}

Drunk.Emotes = { -- me
	"смотрит пьянным взглядом куда-то в пустоту, стараясь разглядеть что-то",
	"старается сосредоточиться и сфокусировать свой мутный взгляд.",
	"икает и кашляет из-за крепости алкоголя.",
	"слегка пошатывается, стараясь держать равновесие."
}
...
function playerMeta:MakeQualityFood(attrib_id)
	...
	local chancedice = math.Clamp(skill*boost.base + math.random(1, 100) * (skill/ix.config.Get("maxAttributes", 10)*boost.chance), 0, 100)

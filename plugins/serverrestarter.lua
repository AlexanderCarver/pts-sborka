...
ix.config.Add("serverRestartHour", 12, "Через сколько часов сервер должен рестартнуться?", function()
    if (SERVER) then
...
function PLUGIN:NotifyServerRestart(client, timeRemaining)
    ...
    client:Notify("Сервер будет перезапущен через "..timeRemainingMinutes.." минут!")
end
...
ix.command.Add("AddServerRestartTime", {
	description = "Прибавить время к рестарту сервера, отстрочив его. В минутах.",
	adminOnly = true,
	...
	OnRun = function(self, client, delay)
        ...
        ix.util.Notify(string.format("Добавлено %d минут отсрочки!", delay))
	end
})
ITEM.name = "Сигареты “Слава”"
ITEM.description = "Пачка сигарет. Всё не так уж плохо на сегодняшний день."
ITEM.longdesc = "Табачные сигареты 1-го сорта, если верить ГОСТ’у. Имеют советскую расцветку фильтра, плотно набиты табаком. При курении возникает отчетливый запах смолы…"
...
ITEM.functions.use = {
	...
	OnRun = function(item)
		...
		ix.util.PlayerPerformBlackScreenAction(item.player, "Употребление...", 8, function(player) 
			...
			ix.chat.Send(player, "me", "достает пачку сигарет Слава и берет одну из них в рот, после чего закуривает, наполняя воздух горьким табачным дымом. Вкус обычного табака.")
		end)
